...
// HELPERS
function newDeadline() { return Date.now() + SLA_SECONDS * 1000; }
...
function showToast(message, type = 'success') {
    ...
    toast.innerHTML = `<i class="fas ${icons[type]}" style="color: ${colors[type]};"></i><span class="toast-msg">${message}</span>`;
...
function setupFirebaseListener() {
    if (!window.fb) { console.error('❌ Firebase not loaded'); setConnectionStatus('error'); return; }
    fbReady = true;
    console.log('🔥 Setting up Firestore listener...');

    const ref = window.fb.collection(window.fb.db, COMPLAINTS_COLLECTION);
    window.fb.onSnapshot(ref, async (snapshot) => {
        complaints = [];
        snapshot.forEach(doc => complaints.push({ ...doc.data(), _docId: doc.id }));
        complaints.sort((a, b) => a.complaint_id - b.complaint_id);
        console.log(`🔥 ${complaints.length} complaints received`);
...
async function seedInitialData() {
    try {
        const snap = await window.fb.getDocs(window.fb.collection(window.fb.db, COMPLAINTS_COLLECTION));
        if (snap.empty) {
            const seedData = [
                { complaint_id: 1001, submitted_by: 'Demo Student', category: 'IT/Wi-Fi', description: 'Wi-Fi not working in Library 2nd floor', priority: 'High', status: 'Open', assigned_staff_id: 1, escalation_level: 0, strict: false, sla_deadline: Date.now() + SLA_SECONDS * 1000, created_at: Date.now() },
                { complaint_id: 1002, submitted_by: 'Demo Student', category: 'Electrical', description: 'Main hall ceiling fan sparking', priority: 'High', status: 'Open', assigned_staff_id: 3, escalation_level: 0, strict: false, sla_deadline: Date.now() + SLA_SECONDS * 1000, created_at: Date.now() }
            ];
            for (const c of seedData) {
                await window.fb.setDoc(window.fb.doc(window.fb.db, COMPLAINTS_COLLECTION, String(c.complaint_id)), c);
            }
        }
...
async function saveTicket(ticket) {
    ...
    const clean = { ...ticket };
    delete clean._docId;
    await window.fb.setDoc(window.fb.doc(window.fb.db, COMPLAINTS_COLLECTION, docId), clean);
}
...
function updateHeaderStats() {
    ...
    setText('staffBadge', complaints.filter(c => c.category === currentStaffDept && (c.status === 'Open' || c.status === 'In Progress')).length);
    setText('hodBadge', complaints.filter(c => c.category === currentHODDept && c.status === 'Escalated to HOD').length);
}
...
function renderStudentTable() {
    ...
    if (complaints.length === 0) {
        tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state"><i class="fas fa-inbox"></i><h4>No complaints yet</h4></div></td></tr>`;
        return;
    }
    ...
    [...complaints].sort((a, b) => b.complaint_id - a.complaint_id).forEach(c => {
        ...
        tbody.innerHTML += `
            <tr>
                ...
                <td><span class="status-badge ${statusClass}">${c.status}</span>${c.strict ? '<span class="strict-badge">STRICT</span>' : ''}</td>
...
function renderStaffTable() {
    ...
    if (filtered.length === 0) {
        tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state"><i class="fas fa-folder-open"></i><h4>No tickets for ${currentStaffDept}</h4></div></td></tr>`;
        return;
    }
    ...
    filtered.sort((a, b) => b.complaint_id - a.complaint_id).forEach(c => {
        ...
        tbody.innerHTML += `
            <tr>
                ...
                <td><span class="status-badge ${statusClass}">${c.status}</span>${c.strict ? '<span class="strict-badge">STRICT</span>' : ''}</td>
                <td id="sla-${c.complaint_id}">${timeLeftLabel(c)}</td>
                <td>${locked ? '<span style="color: var(--text-light);"><i class="fas fa-lock"></i> Locked</span>' : `
                    <select class="status-select" onchange="updateStaffStatus(${c.complaint_id}, this.value)">
                        <option value="Open" ${c.status === 'Open' ? 'selected' : ''}>Open</option>
                        <option value="In Progress" ${c.status === 'In Progress' ? 'selected' : ''}>In Progress</option>
                        <option value="Resolved" ${c.status === 'Resolved' ? 'selected' : ''}>Resolved</option>
                    </select>`}</td>
                <td>${locked || c.status === 'Resolved' ? '<span style="color: var(--text-light);">—</span>' : 
                    `<button class="btn btn-danger btn-xs" onclick="escalateToHOD(${c.complaint_id})"><i class="fas fa-arrow-up"></i> Unable</button>`}</td>
            </tr>`;
    });
}
...
function renderHODTables() {
    ...
    if (escBody) {
        escBody.innerHTML = '';
        const escalated = complaints.filter(c => c.category === currentHODDept && c.status === 'Escalated to HOD');
        if (escalated.length === 0) {
            escBody.innerHTML = `<tr><td colspan="6"><div class="empty-state"><i class="fas fa-check-circle" style="color: var(--success);"></i><h4>No escalated tickets</h4></div></td></tr>`;
        } else {
            escalated.forEach(c => {
                const priorityClass = `priority-${c.priority}`;
                const statusClass = `status-${c.status.replace(/\s/g, '-')}`;
                    ...
                    <tr>
                        ...
                        <td><div class="action-buttons">
                            <select id="hodReassignSelect-${c.complaint_id}">
                                ${getStaffByDept(c.category).map(s => `<option value="${s.staff_id}" ${s.staff_id === c.assigned_staff_id ? 'selected' : ''}>${s.name} (${s.escalation_count})</option>`).join('')}
                            </select>
                            <button class="btn btn-warning btn-xs" onclick="reassignStrict(${c.complaint_id})"><i class="fas fa-exchange-alt"></i> Reassign</button>
                            <button class="btn btn-danger btn-xs" onclick="escalateToAdmin(${c.complaint_id})"><i class="fas fa-arrow-up"></i> To Admin</button>
                        </div></td>
                    </tr>`;
            });
        }
    ...
    if (allBody) {
        ...
        if (allDept.length === 0) {
            allBody.innerHTML = `<tr><td colspan="6" style="text-align:center;color:var(--text-light);padding:20px;">No tickets.</td></tr>`;
        } else {
        ...
        sorted.forEach(s => {
            ...
            perfBody.innerHTML += `
                <tr>
                    ...
                    <td><div class="perf-wrapper"><div class="performance-bar"><div class="fill ${cls}" style="width: ${pct}%;"></div></div><span class="perf-label">${pct.toFixed(0)}%</span></div></td>
                </tr>`;
        });
    }
}
...
function renderAdminTables() {
    ...
    if (escBody) {
        ...
        if (level2.length === 0) {
            escBody.innerHTML = `<tr><td colspan="7"><div class="empty-state"><i class="fas fa-shield-alt" style="color: var(--success);"></i><h4>No Level-2 escalations</h4></div></td></tr>`;
        } else {
            level2.forEach(c => {
                const priorityClass = `priority-${c.priority}`;
                const statusClass = `status-${c.status.replace(/\s/g, '-')}`;
                escBody.innerHTML += `
                    <tr>
                        ...
                        <td><div class="action-buttons">
                            ...
                            <button class="btn btn-success btn-xs" onclick="adminResolve(${c.complaint_id})"><i class="fas fa-check"></i> Resolve</button>
                            <button class="btn btn-warning btn-xs" onclick="adminReassignToDept(${c.complaint_id})"><i class="fas fa-exchange-alt"></i> Reassign</button>
                        </div></td>
                    </tr>`;
            });
        }
    ...
    if (usersBody) {
        ...
        staffMembers.forEach(s => {
            usersBody.innerHTML += `<tr><td><i class="fas fa-user-cog" style="color: var(--info);"></i> Staff</td><td>${s.name}</td><td>${s.department}</td></tr>`;
        });
        hods.forEach(h => {
            usersBody.innerHTML += `<tr><td><i class="fas fa-user-tie" style="color: var(--warning);"></i> HOD</td><td>${h.name}</td><td>All Departments</td></tr>`;
        });
    }
...
async function submitComplaint(e) {
    ...
    if (!desc || !studentName) { showToast('Please fill in all fields.', 'warning'); return; }
    if (!fbReady) { showToast('Not connected. Please wait.', 'error'); return; }

    const btn = $('submitBtn');
    if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner"></i> Submitting...'; }

    try {
        const newId = complaints.length ? Math.max(...complaints.map(c => c.complaint_id)) + 1 : 1001;
        const newTicket = {
            complaint_id: newId, submitted_by: studentName, category: cat, description: desc,
            priority: priority, status: 'Open', assigned_staff_id: findStaffForDept(cat),
            escalation_level: 0, strict: false, sla_deadline: newDeadline(), created_at: Date.now()
        };
        ...
        if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fas fa-paper-plane"></i> Submit Complaint'; }
    }
}
...
async function updateStaffStatus(id, newStatus) {
    const t = complaints.find(c => c.complaint_id === id);
    if (!t) return;
    t.status = newStatus;
    if (newStatus === 'Resolved') t.strict = false;
    try { await saveTicket(t); showToast(`Ticket #${id} → ${newStatus}`, 'info'); }
...
async function escalateToHOD(id) {
    const t = complaints.find(c => c.complaint_id === id);
    if (!t) return;
    t.status = 'Escalated to HOD';
    t.escalation_level = 1;
    incrementEscalationCount(t.assigned_staff_id);
    try { await saveTicket(t); showToast(`⬆️ Ticket #${id} escalated to HOD.`, 'warning'); }
    catch (err) { showToast('Escalation failed.', 'error'); }
}
...
async function reassignStrict(id) {
    ...
    const chosenStaffId = select ? parseInt(select.value) : t.assigned_staff_id;
    t.assigned_staff_id = chosenStaffId;
    t.status = 'Open';
    t.strict = true;
    t.sla_deadline = newDeadline();
    try { await saveTicket(t); showToast(`🔄 Reassigned STRICT to ${getStaffName(chosenStaffId)}.`, 'warning'); }
    catch (err) { showToast('Reassign failed.', 'error'); }
}

async function escalateToAdmin(id) {
    const t = complaints.find(c => c.complaint_id === id);
    if (!t) return;
    t.status = 'Escalated to Admin';
    t.escalation_level = 2;
    try { await saveTicket(t); showToast(`⬆️ Ticket #${id} → Admin.`, 'error'); }
    catch (err) { showToast('Escalation failed.', 'error'); }
}
...
async function adminResolve(id) {
    ...
    t.status = 'Resolved';
    t.strict = false;
    try { await saveTicket(t); showToast(`✅ Ticket #${id} resolved.`, 'success'); }
...
async function adminReassignToDept(id) {
    ...
    t.escalation_level = 1;
    t.sla_deadline = newDeadline();
    try { await saveTicket(t); showToast(`🔄 Reassigned STRICT.`, 'warning'); }
    catch (err) { showToast('Reassign failed.', 'error'); }
}

async function clearAllData() {
    if (!confirm('⚠️ Delete ALL complaints? This affects everyone!')) return;
    try {
        const snap = await window.fb.getDocs(window.fb.collection(window.fb.db, COMPLAINTS_COLLECTION));
        for (const docItem of snap.docs) {
            await window.fb.deleteDoc(window.fb.doc(window.fb.db, COMPLAINTS_COLLECTION, docItem.id));
        }
...
async function checkAndAutoEscalate() {
    for (const c of complaints) {
        ...
        if (stillWithStaff && c.escalation_level < 1 && Date.now() >= c.sla_deadline) {
            ...
            try { await saveTicket(c); showToast(`⏰ SLA breached! Ticket #${c.complaint_id} auto-escalated.`, 'error'); }
            catch (err) { console.error('Auto-escalation failed:', err); }
        }
    }
}
...
setTimeout(() => {
    if (!fbReady && window.fb) setupFirebaseListener();
}, 1500);
